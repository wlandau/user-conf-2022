...
list(
  tar_target(data, get_data(), repository = "aws"),
  tar_target(model, run_model(data), repository = "aws")
)
```
...
* How to configure a cloud-backed version-aware pipeline:
    ...
    3. Select AWS as the storage repository.
    4. Commit `_targets/meta/meta` to source code version control.
